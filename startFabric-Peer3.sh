...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org3.example.com/msp" peer0.org3.example.com peer channel fetch config -o {IP-HOST-1}:7050 -c composerchannel
...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org3.example.com/msp" peer1.org3.example.com peer channel fetch config -o {IP-HOST-1}:7050 -c composerchannel
...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org3.example.com/msp" peer2.org3.example.com peer channel fetch config -o {IP-HOST-1}:7050 -c composerchannel
