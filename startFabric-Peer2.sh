...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org2.example.com/msp" peer0.org2.example.com peer channel fetch config -o {IP-HOST-1}:7050 -c composerchannel --tls --cafile /etc/hyperledger/orderer/msp/tlscacerts/tlsca.{IP-HOST-1}-cert.pem
...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org2.example.com/msp" peer1.org2.example.com peer channel fetch config -o {IP-HOST-1}:7050 -c composerchannel --tls --cafile /etc/hyperledger/orderer/msp/tlscacerts/tlsca.{IP-HOST-1}-cert.pem
...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org2.example.com/msp" peer2.org2.example.com peer channel fetch config -o {IP-HOST-1}:7050 -c composerchannel --tls --cafile /etc/hyperledger/orderer/msp/tlscacerts/tlsca.{IP-HOST-1}-cert.pem
